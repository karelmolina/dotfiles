...
step2_core_deps() {
    ...
    # Add lazygit PPA
    if ! grep -q "lazygit" /etc/apt/sources.list.d/*.list 2>/dev/null; then
        echo_info "Adding lazygit PPA..."
...
step8_apps() {
    ...
    sudo apt install -y git-cola git-delta meld vim bat exa btop podman-docker
